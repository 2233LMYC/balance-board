...
int ESP8266_Server_Init(void)
{
	...
	switch(Step)
	{
		case 0: HAL_UART_Transmit_IT(&huart2,Str1,sizeof(Str1));
						HAL_Delay(1000);
						...
						break;
		case 1: HAL_UART_Transmit_IT(&huart2,Str2,sizeof(Str2));
						HAL_Delay(1000);
						if(strstr((const char*)USART2_REC_Buff,(const char*)"OK") != NULL)
						{
							memset(USART2_REC_Buff,0,strlen((const char*)USART2_REC_Buff));
						...
						break;
		case 2: HAL_UART_Transmit_IT(&huart2,Str3,sizeof(Str3));
						HAL_Delay(1000);
						if(strstr((const char*)USART2_REC_Buff,(const char*)"OK") != NULL)
						{
							memset(USART2_REC_Buff,0,strlen((const char*)USART2_REC_Buff));
						...
						break;
		case 3: HAL_UART_Transmit_IT(&huart2,Str4,sizeof(Str4));
						HAL_Delay(1000);
						if(strstr((const char*)USART2_REC_Buff,(const char*)"OK") != NULL)
						{
							memset(USART2_REC_Buff,0,strlen((const char*)USART2_REC_Buff));
						...
						break;
		case 4: HAL_UART_Transmit_IT(&huart2,Str5,sizeof(Str5));
						HAL_Delay(1000);
						if(strstr((const char*)USART2_REC_Buff,(const char*)"OK") != NULL)
						{
							memset(USART2_REC_Buff,0,strlen((const char*)USART2_REC_Buff));
...
uint8_t Str3[] = {"AT+CWJAP=\"ESP8266\",\"123456789\"\r\n"};//连接WIFI
uint8_t Str4[] = {"AT+CIPSTART=\"TCP\",\"192.168.4.1\",6666\r\n"};//连接服务器IP
...
int ESP8266_Client_Init(void)
{
	...
	switch(Step)
	{
		case 0: HAL_UART_Transmit(&huart2,Str1,sizeof(Str1),1000);
						HAL_Delay(1000);
						if((USART2_REC_Buff[0]=='O')&&(USART2_REC_Buff[1]=='K'))
						{
							memset(USART2_REC_Buff,0,sizeof(USART2_REC_Buff));
							Step = 1;
							printf("AT OK!\r\n");
						}
						else 
